...
class JSONMapperTestsv:XCTestCase {
    ...
    func test_with_valid_json_successfully_decodes() {
        ...
        XCTAssertEqual(userResponse?.data[3].id, 4, "The ID should be 3")
        ...
        XCTAssertEqual(userResponse?.support.url, "https://reqres.in/#support-heading", "The url should be 'https://reqres.in/#support-heading' ")
        XCTAssertEqual(userResponse?.support.text, "To keep ReqRes free, contributions towards server costs are appreciated!", "TThe text should be 'To keep ReqRes free, contributions towards server costs are appreciated!' ")
    ...
    func test_with_missing_file_error_thrown() {
        ...
        do {
            _ = try StaticJSONMapper.decode(file: "FileThatDoesNotExist", type: UsersResponse.self)
        } catch {
    ...
    func test_with_invalid_file_error_thrown() {
        XCTAssertThrowsError(try StaticJSONMapper.decode(file: "FileThatDoesNotExist", type: UsersResponse.self), "An error should be thrown.")
        do {
            _ = try StaticJSONMapper.decode(file: "FileThatDoesNotExist", type: UsersResponse.self)
        } catch {
            guard let mappingError = error as? StaticJSONMapper.MappingError else {
